...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const getBall = async () => {
      const {
        data: { data: balls }
      } = await api_index.apiGetBall();
      state.balls = balls;
    };
    const getRecommendList = async () => {
      const {
        data: { recommend }
      } = await api_index.apiGetRecommendList();
      state.recommendList = recommend;
    };
    const getRecommendSongs = async () => {
      const res = await api_index.apiGetRecommendSongs();
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-1cf27b2a"], ["__file", "D:/WORKSPACE/LM_html/Vue/uniapp/music/pages/index/index.vue"]]);
